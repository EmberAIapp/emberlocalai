...
- **Ingestion des données (à concevoir)** : comment l'utilisateur fait entrer ses données —
  glisser un fichier/dossier (.txt/.md/.pdf), connecteurs locaux en lecture seule
  (Apple Notes, Mail, Obsidian). C'est l'UX cœur du « apprends de moi ».
...
- **MLX (Apple) = moteur d'inférence principal.** PROUVÉ sur M5 : Qwen2.5-1.5B-Instruct-4bit
  → français fluide, **ancre les faits injectés**, **56 tok/s à chaud**. C'est le passage
  « démo → produit ». Le moteur Rust/CoreML reste pour le training perso et l'ANE.
- Modèles cibles : **Qwen2.5-1.5B-Instruct** (libre, multilingue) par défaut, option 3B pour
  plus de qualité ; SmolLM2-Instruct pour l'entrée de gamme (8 Go).
...
- **Inférence ANE** : via Core ML public (computeUnits=All). Dispatch op-par-op = Core ML Performance Report (à fournir avant de communiquer dessus). Secondaire vs MLX au court terme.
...
- **🔴 Signature + notarisation Apple** : requiert un **compte Apple Developer (99 $/an)**. Sans ça, Gatekeeper bloque le téléchargement chez l'inconnu. Gate business à décider tôt.
- **Harnais d'évaluation (golden facts)** : jeu de référence par IA pour mesurer « meilleur/pire » à chaque changement. Prérequis de §9.A (correction supervisée).
- **Licences modèles** : embarquer un modèle a des implications — SmolLM2/Qwen (Apache, OK), Llama (gated/licence). Histoire de licence propre pour la distribution.
- **Mises à jour de l'app** : Sparkle (updates Mac signées).
...
- **Preuve de confidentialité = moat** : viser un entitlement « pas de réseau » sur le cœur
  (ou un moniteur réseau montrant zéro sortie). Le « 100% local » prouvé devient un argument
  de confiance et de pitch, pas une simple promesse.
...
1. Sur 10 prompts FR variés → **≥ 8** réponses fluides/justes.
2. **5/5** faits perso restitués après fermer→rouvrir.
3. **≥ 20 tok/s à chaud** sur le modèle cible (mesuré : 56 → OK).
...
- Rappel mémoire < 4/5 fiable → réparer le cœur avant tout le reste.
...
- **Gratuit** : l'app + le moteur, 100% local → moat privacy + acquisition virale.
- **Payant (~9–12 €/mois)** : synchro **chiffrée** du modèle/mémoire entre appareils ·
  bibliothèque de modèles optimisés en 1 clic · connecteurs locaux (Notes/Mail/Obsidian) ·
  offre pro (batch, équipes).
- **Économie** : local = **zéro coût serveur = forte marge** sur l'abonnement ; la valeur
  payante (synchro, connecteurs, modèles curés) justifie de payer **sans trahir le local**.
- **Distribution** : `.dmg` direct d'abord (liberté agent/fichiers), Mac App Store ensuite
  pour une version lite.
