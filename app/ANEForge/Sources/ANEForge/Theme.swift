...
// MARK: - Agent step mark (shared by Le fil + Mode Her)
// Spec MARK map (logic L565-571).

struct StepMark: View {
    let state: AppState.StepState
    var body: some View {
        ZStack {
            Circle().fill(bg)
            if state == .doing {
                // doing: radial-gradient(circle at 35% 30%,#ffcf9a,#ff6a26 65%,#c42a12)
                Circle().fill(RadialGradient(
                    stops: [.init(color: Color(hexv: 0xffcf9a), location: 0),
                            .init(color: Color(hexv: 0xff6a26), location: 0.65),
                            .init(color: Color(hexv: 0xc42a12), location: 1)],
                    center: UnitPoint(x: 0.35, y: 0.30), startRadius: 0, endRadius: 11))
            } else {
                Text(glyph).font(.system(size: 11, weight: .bold)).foregroundStyle(fg)
            }
        }
        .frame(width: 18, height: 18)
        // doing glow: 0 0 10px rgba(255,110,50,0.75)
        .shadow(color: state == .doing ? Color(hexv: 0xff6e32).opacity(0.75) : .clear, radius: 5)
    }
    private var bg: Color {
        switch state {
        case .done:    return Color(hexv: 0x5fd07a).opacity(0.16)
        case .doing:   return .clear
        case .gate:    return Color(hexv: 0xffc850).opacity(0.20)
        case .pending: return .white.opacity(0.05)
        case .denied:  return .white.opacity(0.04)
        }
    }
    private var fg: Color {
        switch state {
        case .done:    return Color(hexv: 0x7fd095)
        case .doing:   return .white
        case .gate:    return Color(hexv: 0xffd089)
        case .pending: return Color(hexv: 0x7c6f67)
        case .denied:  return Color(hexv: 0x8a7d75)
        }
    }
    private var glyph: String {
        switch state {
        case .done: return "✓"; case .doing: return ""; case .gate: return "!"
        case .pending: return "·"; case .denied: return "✕"
        }
    }
}

extension AppState.StepState {
    // Step text colors (MARK.tcol, logic L565-571)
    var textColor: Color {
        switch self {
        case .done:    return Color(hexv: 0xa99a8f)
        case .doing:   return Color(hexv: 0xf0ddcf)
        case .gate:    return Color(hexv: 0xf0ddcf)
        case .pending: return Color(hexv: 0x7c6f67)
        case .denied:  return Color(hexv: 0x8a7d75)
        }
    }
    // Status word (STATUS map, logic L572)
    var statusText: String {
        switch self {
        case .doing: return "en cours"; case .pending: return "en attente"
        case .denied: return "refusé"; default: return ""
        }
    }
    var showsStatus: Bool { self == .doing || self == .pending || self == .denied }
}
